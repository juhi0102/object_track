...
def mouse_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if x_pos < 200:  # Move left
            drone_controller.drone.rcRoll = mapping(x_pos, 0, 200, 1000, 1300)
            drone_controller.drone.left()
            print("Moving forward:", drone_controller.drone.rcRoll)
        elif x_pos > 500:  # Move right
            drone_controller.drone.rcRoll = mapping(x_pos, 500, 600, 1600, 2000)
            drone_controller.drone.right()
            print("Moving bacward:", drone_controller.drone.rcRoll)
