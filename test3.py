        ...
        for event in pygame.event.get():
            if event.type == pygame.JOYAXISMOTION:
                ...
                if event.button == 0:  # Button A (arm)
                    ...
                    me.disarm()
                elif event.button == 2:  # Button Start (take off)
                    me.take_off()
                elif event.button == 3:  # Button Back (land)
                    me.land()

            elif event.type == pygame.JOYBUTTONUP:
                # Button release event handling
                if event.button in [0, 1, 2, 3]:  # Arm, disarm, take off, land
                    me.reset()
