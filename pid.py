...
pid_roll = PID(Kp=0.1, Ki=0.01, Kd=0.05, setpoint=0)
pid_pitch = PID(Kp=0.1, Ki=0.01, Kd=0.05, setpoint=0)
pid_yaw = PID(Kp=0.1, Ki=0.01, Kd=0.05, setpoint=0)
...
while True:
    ...
    roll_error =  2.58
    # pitch_error =  # Calculate pitch error
    # yaw_error =  # Calculate yaw error

    # Update PID controllers
    roll_output = pid_roll.update(roll_error)
    # pitch_output = pid_pitch.update(pitch_error)
    # yaw_output = pid_yaw.update(yaw_error)

    # Adjust Pluto commands based on PID outputs
    me.rcRoll = roll_output
    # me.rcPitch = pitch_output
    # me.rcYaw = yaw_output
