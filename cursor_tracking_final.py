...
def mouse_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        ...
        if x_pos < 200:  # Move left
            drone_controller.drone.rcRoll = mapping(x_pos, 0, 200, 1300, 1500)
            print("Moving left:", drone_controller.drone.rcRoll)
        elif x_pos > 500:  # Move right
            drone_controller.drone.rcRoll = mapping(x_pos, 500, 700, 1500, 1700)
            print("Moving right:", drone_controller.drone.rcRoll)
        else:  # Center position
            drone_controller.drone.rcRoll = 1500
            print("Center position")

        if y_pos < 200:  # Move up
            drone_controller.drone.rcPitch = mapping(y_pos, 150, 0, 1500, 1700)
            print("Moving forward:", drone_controller.drone.rcPitch)
        elif y_pos > 300:  # Move down
            drone_controller.drone.rcPitch = mapping(y_pos, 300, 500, 1500, 1300)
